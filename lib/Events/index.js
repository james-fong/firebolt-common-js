  ...
  if (typeof callback !== 'function') {
    ...
    events.forEach(function (event) {
      ...
      if (Object.values(listeners.get(key)).length === 0) {
        var _Transport$listen = _index["default"].listen(module, 'on' + event[0].toUpperCase() + event.substring(1), {
          listen: true
        }),
...
var clear = function clear() {
  ...
  if (args && args.length && typeof args[0] === 'number') {
    ...
    var moduleOrId = args.shift();
    var event = args.shift() || undefined;
...
var unsubscribe = function unsubscribe(key) {
  ...
  _index["default"].send(module, 'on' + event[0].toUpperCase() + event.substr(1), {
    listen: false
  });

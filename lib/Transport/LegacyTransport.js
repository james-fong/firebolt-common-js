...
var LegacyTransport = /*#__PURE__*/function () {
  ...
  _createClass(LegacyTransport, [{
    ...
    value: function receive(callback) {
      ...
      var badgerCallback = window.$badger.callback ? window.$badger.callback.bind(window.$badger) : null;
      var badgerEvent = window.$badger.event ? window.$badger.event.bind(window.$badger) : null;

      window.$badger.callback = function (pid, success, json) {
        if (json.jsonrpc) {
      ...
      window.$badger.event = function (handlerId, json) {
        if (json.jsonrpc) {
          callback(JSON.stringify(json));
        } else if (badgerEvent) {
